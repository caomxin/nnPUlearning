...
class DrugTargetNetwork(MyClassifier, Chain):
    def __init__(self, prior, dim):
        super(DrugTargetNetwork, self).__init__(
            ...
            l5=L.Linear(50, 1),
            l6=L.Linear(50, 1),
            w1=chainer.Variable(np.array(np.random.rand(1)), dtype=np.float32),
            w2=chainer.Variable(np.array(np.random.rand(1)), dtype=np.float32),
            b=chainer.Variable(np.array(np.random.rand(1)), dtype=np.float32)
        )
        self.af = F.relu
        self.prior = prior
    ...
    def calculate(self, x):
        """
            x[i, :1000] is drug inCHI with shape([batchsize, 1000])
            x[i, 1000:] is target amino acid sequence with shape[batchsize, 1500]
        """
        h1=self.l1_1(x[:, :1000])
        h1=self.l1_2(h1)
        h1=self.l3(h1)
        ...
        h=self.w1 * h1 + self.w2 * h2 + self.b
